...
impl BigqueryRequestBuilder {
    fn build_proto_data(
        ...
        // `ProtobufSerializer` exposes `DescriptorProto` from `prost-reflect`'s bundled
        // `prost-types`, which is a different version than the one tonic generates against.
        // The wire format is identical, so round-trip through bytes to convert.
        let descriptor_bytes = prost_reflect::prost::Message::encode_to_vec(
            self.protobuf_serializer.descriptor_proto(),
        );
        let proto_descriptor = prost_types::DescriptorProto::decode(&*descriptor_bytes)
            .expect("DescriptorProto round-trip through bytes is infallible");
